            ...
            await Task.Run(async () =>
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        {
                            Debug.WriteLine($"{nameof(this.ProcessCommandQueue)}: command queue is {this.commands.Count} deep.");

                            var request = new TelloRequest(command);
                            Debug.WriteLine($"{nameof(this.ProcessCommandQueue)}: request.Message '{request.Message}'");
                            Debug.WriteLine($"{nameof(this.ProcessCommandQueue)}: request.Timeout '{request.Timeout}'");

                            var response = new TelloResponse(await this.transceiver.SendAsync(request));
                            Debug.WriteLine($"{nameof(this.ProcessCommandQueue)}: response.Success '{response.Success}'");
                            Debug.WriteLine($"{nameof(this.ProcessCommandQueue)}: response.Message '{response.Message}'");
